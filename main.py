...
def get_spotify_client():
    """Create and cache the Spotify client on first use."""
    global sp, user
...
def main(start_index, end_index, cutoff_date):
    ...
    try:
        followed_artists = get_and_cache_followed_artists(spotify_client)
    ...
    def process_artist(artist_info):
        ...
        try:
            return get_new_tracks(spotify_client, artist['id'], artist_name, cutoff_date)
        except Exception as e:
    ...
    playlist = create_playlist_with_tracks(spotify_client, all_tracks, start_index, actual_end)
